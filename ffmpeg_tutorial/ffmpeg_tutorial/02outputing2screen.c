...
int main(int argc, char *argv[]) {
	...
	unsigned char	*pYUVDataBuffer = NULL;
	AVPacket        packet;
	int             frameFinished;
	float           aspect_ratio;
...
//	SDL_Overlay     *bmp;
	...
	pYUVDataBuffer = (unsigned char *)av_malloc(av_image_get_buffer_size(AV_PIX_FMT_YUV420P, pCodecCtx->width, pCodecCtx->height, 1));
...
// 		screen);
	...
	screen = SDL_CreateWindow("Simplest ffmpeg player's Window", SDL_WINDOWPOS_UNDEFINED, 
		SDL_WINDOWPOS_UNDEFINED, pCodecCtx->width, pCodecCtx->height, SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE);
	if (!screen) {
