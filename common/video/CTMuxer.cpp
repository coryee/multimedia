...
CTMuxer::~CTMuxer()
{
	...
	for (iter = m_outputs.begin(); iter != m_outputs.end(); iter++) {
		//Write file trailer
		if (m_inited) {
			av_write_trailer(iter->ofmt_ctx);
		}
		if(iter->ofmt_ctx && !(iter->ofmt_ctx->oformat->flags & AVFMT_NOFILE))
			avio_close(iter->ofmt_ctx->pb);
		avformat_free_context(iter->ofmt_ctx);
	}
	if (m_h264bsfc) {
...
int CTMuxer::AddOutputFileName(const char *output_filename)
{
	...
	vector<CTMuxerOutput>::iterator iter;
	for (iter = m_outputs.begin(); iter != m_outputs.end(); iter++) {
		if (strcmp(iter->filename, output_filename) == 0) {
			printf("url[%s] has already existed in CTMuxer\n", output_filename);
			return 0;
		}
	}

	CTMuxerOutput output;
...
int CTMuxer::AddInputStreamInfo(int stream_type, AVCodecContext *codec_ctx, AVRational time_base)
{
	...
	vector<CTMuxerOutput>::iterator iter;
	for (iter = m_outputs.begin(); iter != m_outputs.end(); iter++) {
		AVStream *stream = CreateStream(iter->ofmt_ctx, codec_ctx);
		if (!stream) {
			printf("Failed create video stream\n");
			return -1;
		}
		iter->streams[stream_type] = stream;
	}
...
int CTMuxer::AddInputStreamInfo(int stream_type, CTMuxerStreamInfo *stream_info)
{
	...
	vector<CTMuxerOutput>::iterator iter;
	for (iter = m_outputs.begin(); iter != m_outputs.end(); iter++) {
		AVStream *stream = CreateStream(iter->ofmt_ctx, stream_info);
		if (!stream) {
			printf("Failed create video stream\n");
			return -1;
		}
...
int CTMuxer::Mux(int stream_type, AVPacket *input_pkt)
{
	...
	for (iter = m_outputs.begin(); iter != m_outputs.end(); iter++) {
		...
		AVStream *out_stream = iter->streams[stream_type];
		if (stream_type == CTMUXER_STREAM_TYPE_AUDIO && m_aacbsfc &&
			iter->need_audio_bitstream_filter) {
			ret = av_bitstream_filter_filter(m_aacbsfc, out_stream->codec, NULL, &pkt->data, &pkt->size, pkt->data, pkt->size, 0);
		} else if (stream_type == CTMUXER_STREAM_TYPE_VIDEO && m_h264bsfc &&
			iter->need_video_bitstream_filter) {
		...
		//Write
		int ret = av_interleaved_write_frame(iter->ofmt_ctx, pkt);
		av_packet_unref(pkt);
		if (ret < 0) {
...
int CTMuxer::Init()
{
	...
	for (iter = m_outputs.begin(); iter != m_outputs.end(); iter++) {
		...
		av_dump_format(iter->ofmt_ctx, 0, iter->filename, 1);
		printf("======================================\n");

		//Open output file
		if (!(iter->ofmt_ctx->oformat->flags & AVFMT_NOFILE)) {
			if (avio_open(&iter->ofmt_ctx->pb, iter->filename, AVIO_FLAG_WRITE) < 0) {
				printf("Could not open output file '%s'", iter->filename);
				return -1;
			}
		}

		avformat_write_header(iter->ofmt_ctx, NULL);
	}
