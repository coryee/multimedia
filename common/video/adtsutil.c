...
int aac_set_adts_head(ADTSContext *acfg, unsigned char *buf, int frame_length)
{
	unsigned char byte;
	...
	byte |= (ADTS_HEADER_SIZE + frame_length) >> 11;
	buf[3] = byte;
	byte = 0;
	byte |= (ADTS_HEADER_SIZE + frame_length) >> 3;
	buf[4] = byte;
	byte = 0;
	byte |= ((ADTS_HEADER_SIZE + frame_length) & 0x7) << 5;
	byte |= (0x7ff >> 6) & 0x1f;
	buf[5] = byte;
	byte = 0;
	byte |= (0x7ff & 0x3f) << 2;
