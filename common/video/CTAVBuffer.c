...
void CTAVPacketQueueInit(CTAVPacketQueue *pQueue, int iQueueSize)
{
	...
	pQueue->pMutex = CTCreateMutex();
}

void CTAVPacketQueueDeInit(CTAVPacketQueue *pQueue)
{
	if (pQueue->pMutex)
	{
		CTDestroyMutex(pQueue->pMutex);
		pQueue->pMutex = NULL;
	}
}
...
void CTAVFrameBufferDeInit(CTAVFrameBuffer *pFrameBuffer)
{
	...
	if (pFrameBuffer->pFrames)
	{
		for (i = 0; i < pFrameBuffer->iSize; i++)
		{
			...
			{
				av_frame_free(&pFrameBuffer->pFrames[i].pFrame);
...
CTAVFrame* CTAVFrameBufferFirstFrame(CTAVFrameBuffer *pFrameBuffer)
{
	...
	CTAVFrame *pFrame = CTAVFrameBufferPeekFirstFrame(pFrameBuffer);
	if (pFrame != NULL)
